...
满足任一即触发：
1. 用户明确说"wiki编译"
2. raw/ 哈希变化 — 检测到新增或修改的文件
3. thinking/ 编译完成 — 有新内容落入 wiki/，顺势重新综合 raw + 新编译内容
...
**触发条件**（满足任一即触发）：
1. 用户明确说"话题结束"
2. 自动检测到话题切换：用户未说"话题结束"但对话方向已转向其他主题（如从讨论 A 概念转向讨论 B 概念、转向 Q&A 查询、或转向其他操作）

**自动检测规则**：
- 对话中连续 2 轮以上不再追问同一话题的细节 → 判定为话题切换
- 用户请求的操作与当前 thinking 主题无关（如请求编译 raw、请求 Linting）→ 判定为话题切换
